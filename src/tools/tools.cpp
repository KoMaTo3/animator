...
----------
*/
__int64 GetFileWriteTime( const std::string& file )
{
  ...
  return __int64( mktime( &timestamp ) );
}//getFileWriteTime
...
----------
*/
void Dump( void *mem, unsigned int size, const std::string& comment )
{
  ...
  unsigned int sub;
  for( unsigned int num = 0; num < size; num += 16 )
  {
...
Vec4 StringToColor( const std::string& str ) {
  ...
  if( str.length() != 8 ) {
    ...
    unsigned char *data = ( unsigned char* ) newStr.c_str();
    color.Set(
